...
| `.erb`, `.rhtml` and `.herb` files in `erb/` | **36,781** from 267 apps |
| Size of `erb/` | ~56 MB |
...
**42 of 327** applications use Herb: `herb` 32, `@herb-tools/linter` 19, `@herb-tools/formatter` 12, `.herb.yml` 28.
...
**12 of 327** applications use ReActionView: `reactionview` 12.
...
xychart
  ...
  y-axis "Apps" 0 --> 43
  line [2, 7, 11, 16, 19, 21, 22, 26, 29, 30, 35, 37, 42]
  line [0, 0, 2, 5, 7, 7, 7, 8, 9, 10, 10, 11, 12]
```

Herb 42 · ReActionView 12

### Herb releases in use

```mermaid
pie showData
  title Herb releases across 30 apps
  "0.10.3" : 4
  "0.10.2" : 16
  "0.10.1" : 7
  "0.9.4" : 1
...
Of the 28 Herb configs, the sections people set are `linter` (25), `formatter` (24), `version` (20), `files` (13), `framework` (4), `engine` (4), `template_engine` (2).

`formatter.maxLineLength`: **80** (14), **120** (6)
`formatter.indentWidth`: **2** (20)
